...
def _normalize_claim_references(items: list[object]) -> list[str]:
    """Stabilize claim evidence ids across deterministic replays.

    UUID-shaped ids are generated by lower-level memory entries and do not
    carry semantic meaning for M2.10 profile checks. Replacing them with a
    fixed token removes incidental nondeterminism while keeping structural
    reference counts stable.
    """
    normalized: list[str] = []
    for item in items:
        token = str(item)
...
def _canonicalize_final_identity(payload: dict[str, object]) -> dict[str, object]:
    canonical = json.loads(json.dumps(payload, ensure_ascii=True))

    def _normalize(value: object) -> object:
        if isinstance(value, str):
            return "<uuid>" if _UUID_RE.match(value) else value
    ...
    if isinstance(claims, list):
        for claim in claims:
            ...
            contradicted = claim.get("contradicted_by")
            if isinstance(supported, list):
                claim["supported_by"] = _normalize_claim_references(supported)
            if isinstance(contradicted, list):
                claim["contradicted_by"] = _normalize_claim_references(contradicted)
    return normalized
...
def run_profile_trial(
    *,
    ...
    return {
        ...
        "final_identity": _canonicalize_final_identity(
...
def run_longitudinal_stability(
    *,
    ...
    for profile_index, profile_name in enumerate(profiles):
        ...
        # M2.10 stability should tolerate social/exploratory profiles whose
        # narrative consistency is lower than threat-focused profiles but still
        # far from collapse.
        narrative_ok = min(metrics["narrative_consistency_proxy"]) >= 0.60
