        ...
        if not memory_decision.episode_created:
            ...
            if action.name not in recent_episode_actions or (self.cycle - last_episode_cycle) >= 2:
                ...
                memory_decision = MemoryDecision(
                    ...
                    future_path_utility=float(payload.get("future_path_utility", memory_decision.future_path_utility)),
                    reuse_gain=float(payload.get("reuse_gain", memory_decision.reuse_gain)),
                    error_avoidance_gain=float(payload.get("error_avoidance_gain", memory_decision.error_avoidance_gain)),
                    maintenance_cost=float(payload.get("maintenance_cost", memory_decision.maintenance_cost)),
